# Efficient Sequence Alignment Algorithm - Revised
...
def compute_last_row(x, y):
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            cost1 = prev[j - 1] + mismatch_penalty[x[i - 1] + y[j - 1]]
            cost2 = prev[j] + gap_penalty
            cost3 = curr[j - 1] + gap_penalty
            curr[j] = min(cost1, cost2, cost3)
...
def efficient(x, y):
    if len(x) == 0:
        ...
        return basic_algorithm(x, y)

    xlen = len(x)
    xmid = xlen//2
    score_l = compute_last_row(x[:xmid], y)
    score_r = compute_last_row(x[xmid:][::-1], y[::-1])
    total = [score_l[i] + score_r[len(y) - i] for i in range(len(y) + 1)]
    ymid = total.index(min(total))
    left_alignment = efficient(x[:xmid], y[:ymid])
    right_alignment = efficient(x[xmid:], y[ymid:])
    return left_alignment[0] + right_alignment[0], left_alignment[1] + right_alignment[1]


# base case
def basic_algorithm(x, y):
    m, n = len(x), len(y)
...
def calculate_alignment_cost(aligned_x, aligned_y):
    total_cost = 0
    for x_char, y_char in zip(aligned_x, aligned_y):
        if x_char == '_' or y_char == '_':
            total_cost += gap_penalty
        elif x_char != y_char:
            total_cost += mismatch_penalty.get(x_char + y_char, 0)
    return total_cost


def efficient_algorithm(filename: str):
    [x, y] = input_string_generator(filename)
    aligned_x, aligned_y = efficient(x, y)
    opt_cost = calculate_alignment_cost(aligned_x, aligned_y)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python string_return_efficient.py input.txt output.txt")
